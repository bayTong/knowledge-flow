...
- **完成 R0 初始化所有权加固**：R0.1 提交 `92a37b3` 将自有初始化事务的 marker 固定为最后删除，并验证清理中断后的跨进程恢复；R0.2 提交 `79515ed` 以完整请求哈希和事务 UUID 命名配置临时文件，只清理身份与规范字节均匹配的候选。普通与严格 `ResourceWarning` 全量测试现为 148 项（捕获内核 141 项、维护脚本 7 项）。
