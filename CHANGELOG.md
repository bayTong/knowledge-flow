...
- **完成 C7-0 受限 CLI 编码前契约收口**：在既有操作契约、实现矩阵、编码方案和冲突登记中冻结 `knowledgeflow-capture` 四个精确命令、请求字段白名单、65,536 byte 严格 JSON 头、正文/EOF 预验证、统一响应头、0/2/70 退出码、stderr/日志脱敏，以及生产 `PathPolicy` 与私有测试能力隔离；登记 C-055–C-058 和 CLI-01–CLI-26，并把实现拆为独立 C7A/C7B/C7V。五个状态锚点及文档护栏中唯一的当前门禁断言同步为 C7A，未增加、删除或弱化测试；未创建 `cli.py`、console entry、生产配置或生产 Store，测试基线仍为 274 项。本批已由独立本地提交 `dbe8329` 封存，尚未 push。
- **完成 C7A 私有协议能力与安全原语的内容、本地验证与独立版本化**：新增不导出、无 `main()` 的 `src/knowledgeflow_capture/cli.py` 私有 runner，严格处理四操作参数、65,536 byte JSON 头、字段白名单、UTF-8 正文及精确 EOF；写操作先经受信 `PathPolicy` 和既有配置解析器预检上限，再以不超过 1 MiB 的块写入 Store 外独占磁盘 spool。`get_capture` 使用独立输出 spool，规范响应保持核心 `to_dict()` 并固定 0/2/70 与脱敏 stderr。新增 19 项测试覆盖 C7A 可证明的 CLI-01–CLI-13、CLI-17、CLI-19–CLI-24 边界后，本地全量为 293 项（捕获测试 278 项、脚本测试 15 项），普通与严格 `ResourceWarning` 均通过。未修改 `pyproject.toml`、未安装 console entry、未连接真实四操作、未创建生产配置或 Store；本批已独立本地提交且尚未 push，下一功能门禁为需单独授权的 C7B。
