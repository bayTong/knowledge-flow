...
- **完成 C5-0 追加写入契约收口**：冻结关键字追加请求、独立精确成功结果、`1..999998` expected 上界和必填幂等 key；明确锁内完整性/幂等/目标/CAS 优先级、已提交同 key 命中优先于 CAS、同请求唯一 N+1 尾部窄续封、Event rename 三态现场证据、追加投影时间语义、独立追加 staging 所有权和 APP-01–APP-24。同步 C-035–C-042 与 C5A/C5B/C5V 停点；未修改生产代码、磁盘 schema/golden 或公开 API，当前全量仍为 214 项。本批由独立本地提交 `ea530ad` 闭合，尚未 push；C5A 尚未授权。
