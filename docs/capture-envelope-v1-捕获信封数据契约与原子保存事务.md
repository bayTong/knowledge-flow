...
1. C0–C2 里程碑为 85 项测试，稳定化后为 93 项；C3A/C3B 完成后为 122 项，C3C 完成后为 140 项，C3V 完成后为 144 项，R0.1/R0.2 初始化所有权加固后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后为 231 项，C5B 后当前全量 250 项测试通过。除原有 C3/C4/C5A 证据外，完整追加事务、幂等/CAS、尾部续封、Event 三态和投影降级也已有定向测试。
2. 当前实现已包含公开 `capture_text`、C4B `get_capture`、C4C `list_captures` 和 C5B `append_capture_version`；前三者已经阶段验收，追加仍待 C5V 双进程与真实大小验收。业务崩溃恢复、重建与迁移尚未实现，因此本文整体仍不是 `Effective`。
...
4. C4V `1e38f2f`、C5-0 `ea530ad` 与最小 Windows CI `c4d2c7b` 已同步至 `origin/main`，首次远端 Windows CI 已通过；C5A `63a3250` 与 C5B 均为尚未 push 的独立本地批次。下一功能门禁为仍需另行授权的 C5V，之后按 C6 逐批验收迁移和业务事务崩溃恢复。上述阶段均不接 GBrain。
