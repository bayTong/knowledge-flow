...
> 边界：本文定义完整数据和事务契约；四个公开操作及 C6A–C6C 已实现，C7A 已完成不安装的私有 CLI 协议层，C7B–C8 及本文其余远期能力尚未完成
...
1. C0–C2 里程碑为 85 项测试，稳定化后为 93 项；C3A/C3B 完成后为 122 项，C3C 完成后为 140 项，C3V 完成后为 144 项，R0.1/R0.2 初始化所有权加固后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后为 231 项，C5B 后为 250 项，C5V 后为 253 项，C6A 后为 258 项，C6B 后为 265 项，C6C 后为 274 项，C7A 后当前全量 293 项测试通过。除原有 C3–C5 证据外，事务租约、16 个业务进程终止边界、REC-01–REC-03、MIG-01–MIG-05 以及 C7A 严格帧/有界 spool/退出码与脱敏单元证据也已建立。
...
4. C5A `63a3250`、C5B `ab2a613`、C5V `a9913e2`、C6A `84ee1d7`、C6B `f686941` 与 C6C `ea8f84e` 已于 2026-09-18 同步至 `origin/main`，Windows CI 运行 `35319645501` 首次通过。C7-0 已以本地提交 `dbe8329` 封存，C7A 已完成本地内容、验证与独立版本化但尚未 push；下一功能门禁为需单独授权的 C7B。上述阶段均不接 GBrain。
