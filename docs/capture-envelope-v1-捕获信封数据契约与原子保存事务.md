...
> 状态：Approved Design；四个公开文本操作、C5V、C6A 与 C6B 派生状态重建均已完成<br>
> 整理日期：2026-09-01<br>
> 确认日期：2026-09-01<br>
...
> 边界：本文定义完整数据和事务契约；四个公开操作及 C6A/C6B 已实现，C6C 迁移及本文其余远期能力尚未完成
...
1. C0–C2 里程碑为 85 项测试，稳定化后为 93 项；C3A/C3B 完成后为 122 项，C3C 完成后为 140 项，C3V 完成后为 144 项，R0.1/R0.2 初始化所有权加固后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后为 231 项，C5B 后为 250 项，C5V 后为 253 项，C6A 后为 258 项，C6B 后当前全量 265 项测试通过。除原有 C3–C5 证据外，事务租约、16 个业务进程终止边界，以及 REC-01–REC-03、写前完整验证和中断续建也已有证据。
2. 当前实现已包含公开 `capture_text`、C4B `get_capture`、C4C `list_captures` 和 C5B `append_capture_version`；四者均已完成各自阶段验收，C6A 业务崩溃恢复和 C6B 派生状态重建也已完成。迁移尚未实现，因此本文整体仍不是 `Effective`。
...
4. C4V `1e38f2f`、C5-0 `ea530ad` 与最小 Windows CI `c4d2c7b` 已同步至 `origin/main`，首次远端 Windows CI 已通过；C5A `63a3250`、C5B `ab2a613`、C5V、C6A `84ee1d7` 与 C6B 均为尚未 push 的独立本地批次。下一功能门禁为 C6C Store 迁移；上述阶段均不接 GBrain。
