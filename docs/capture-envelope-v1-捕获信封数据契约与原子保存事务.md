...
1. C0–C2 里程碑为 85 项测试，稳定化后为 93 项；C3A/C3B 完成后为 122 项，C3C 完成后为 140 项，C3V 完成后为 144 项，R0.1/R0.2 初始化所有权加固后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后当前全量 231 项测试通过。除原有 C3/C4 证据外，追加请求/回执、严格 Event writer、版本 2 投影、独立 staging/清理、尾部身份和提交证据原语也已有定向测试。
...
4. C4V `1e38f2f`、C5-0 `ea530ad` 与最小 Windows CI `c4d2c7b` 已同步至 `origin/main`，首次远端 Windows CI 已通过。C5A 内容与本地验证已完成且未公开 append；下一功能门禁为仍需另行授权的 C5B，之后按 C5B/C5V、C6 逐批验收完整追加、迁移和业务事务崩溃恢复。上述阶段均不接 GBrain。
