...
> 状态：Approved Design；C0–C2 与 C3A–C3C 已完成，`capture_text` Item 事务已实现但尚待 C3V 验收<br>
> 整理日期：2026-09-01<br>
> 确认日期：2026-09-01<br>
...
> 边界：本文定义完整数据和事务契约；C3C 只实现新建文本 Capture 的事务，C3V、追加、业务崩溃恢复和本文其余远期能力尚未完成
...
1. C0–C2 里程碑为 85 项测试，稳定化后为 93 项；C3A/C3B 完成后为 122 项，C3C 完成后全量 140 项测试通过。Envelope/Event/Projection schema、受限 YAML、自哈希、配置/Store 初始化、Store 写锁、有界写入、staging 清理及完整新建 Item 事务均已有测试。
2. 当前实现已包含公开 `capture_text`、版本 1、`capture.created`、幂等扫描和初始投影，但 C3V、读取、追加、业务崩溃恢复、重建与迁移尚未完成，因此本文整体仍不是 `Effective`。
3. [C3-0 阻塞性行为决策](c3-0-blocking-behavior-decisions-C3-0阻塞性行为决策.md)已于 2026-09-08 获批，C3A/C3B 已于 2026-09-10 分别完成，C3C 已于 2026-09-11 在测试持有的 Store 中完成；当前仍需用户另行明确授权 C3V，不得创建生产 Store。
4. C3V 验收真实 4 MiB 阈值两侧与 64 MiB 上限；再按 C4–C6 逐批验收读取、追加、迁移和业务事务崩溃恢复。上述阶段均不接 GBrain。
