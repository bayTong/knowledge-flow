...
1. 实现拆解与编码执行方案已经批准；C3 已完成，完整 `capture_text` 事务已在测试持有的 Store 中通过真实大小、加强版竞态与故障验收；R0.1/R0.2、D0-F、D0G、C4-0、R0.3D 与 R0.3F 也已闭合。C4A 已把 Event 证明的读取可见性、请求/结果、游标与 warning 规则落实为内部 schema、模型、codec、纯原语和两版本 golden；新增 19 项后当前全量 182 项通过，并由独立本地提交闭环且未 push。公开读取仍不存在，C4B 尚未授权。
