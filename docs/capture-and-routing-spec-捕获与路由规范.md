...
1. 实现拆解与编码执行方案已经批准；C3 已完成，完整 `capture_text` 事务已在测试持有的 Store 中通过真实大小、加强版竞态与故障验收；R0.1/R0.2、D0-F、D0G、C4-0、R0.3D 与 R0.3F 也已闭合。C4A 读取契约能力与 C4B `get_capture` 已分别以 `06cff02`、`666ba18` 完成并 push；C4C `list_captures` 已完成实现、LIST-01–LIST-19 本地验证和独立本地提交，当前全量 212 项通过，尚未 push。C4V 及后续批次未授权。
