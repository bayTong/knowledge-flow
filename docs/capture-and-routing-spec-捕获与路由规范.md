...
任意入口
  -> 本规范：捕获、暂存、路由、选择处理方式
  -> 需要新 KB 时：SOP-000A
  -> 需要深度语义处理时：先形成带范围、证据和成本边界的处理提案；具体 Profile 待独立门禁冻结
...
处理方式回答“进入待处理范围后，下一步做什么”。Capture v1 只表达用户意图；进入语义处理后，还需要另行选择并记录实际处理策略。当前 Draft 使用 `Processing Profile` 描述完整策展、分层策展和检索优先三类候选路径，但具体名称、默认选择和自动路由尚未冻结。最小意图分为：
...
QQ / App / 浏览器 / 文件 / API / 对话 / 语音
 ...
 GBrain 异步未审核镜像    路由/处理提案
     ...
     已有 KB      新建 KB       暂缓/归档
        ...
        └──────┬────┘
               v
     capture-only / raw-source / 按届时获批策略处理
```
...
下表所有输入都必须先形成由本地 Capture Store 持有的规范原件；“Global Intake”“KB Inbox”和“GBrain”描述的是保存后的逻辑呈现、路由或可选镜像，不是替代第一持久化位置。

| 输入 | 用户未指定 KB 时 | 用户明确指定 KB 时 | 默认处理轨道 |
|---|---|---|---|
| 临时灵感、随手笔记 | 在 Global Intake 视图呈现；可选异步镜像到 GBrain 未审核范围 | 记录目标意图并在 KB `inbox/` 逻辑队列呈现；可选异步镜像 | `capture-only` |
...
| 长文、论文、研究资料 | 先保存，等待选定 KB | 先进入 KB 原料链；用户要求深度整理或命中届时获批的人工规则时，记录 `deep-curation` 意图并进入深层处理门禁 | `raw-source`；满足触发条件后转 `deep-curation` |
| API/自动搜集结果 | 保存原始响应和来源，不信任机器论断 | 进入 KB 的搜集待审队列或 `raw/` | `raw-source` + 主张提案 |

表中的“默认轨道”只决定保存后的建议动作。`deep-curation` 是用户处理意图，不等价于某个已经批准的 Processing Profile，也不授权直接运行现行旧版 SOP-001。未来深层处理只能在 Profile/证据契约和 SOP-001 重构门禁通过后，由一次性用户意图、后续人工选择或用户可见且可撤销的规则触发，以免“只想收藏”也自动产生模型成本。
...
| `deep-curation` | 用户允许进入较深语义处理 | 可先完成 `raw-source`；后续只能按届时获批的处理策略生成带范围和证据的语义提案 | 人审证据和精确变更后再进入 SOP-000B/SOP-002 |
| `promotion-proposal` | 为短想法/捕获生成可信知识候选稿 | 只能写入 `proposals/promotions/` | 人审精确内容后执行 |

其中前三个值是 Capture v1 已冻结的 `user_intent.processing_mode`；`promotion-proposal` 是路由/提案层概念，`Processing Profile` 目前还是方法 Draft。二者都不能直接作为当前 Capture v1 输入值写入原件。

默认规则：

- 灵感、随手笔记默认 `capture-only`。
- 外部文件和 URL 默认建议 `raw-source`。
- 用户明确要求深度整理时，原料归档完成后可直接进入深层处理路由，不再增加一次事前内容审批；实际运行哪种 Profile 仍需在结果中可见。
...
不运行 SOP-001 的长文不会丢失：未分配时留在 Global Intake，已分配时停在 KB Inbox 或 `raw-source`；它可以先建立可重建的区段台账和检索底座，也可以在 `capture/source` 范围检索，但不会因此产生可信 wiki 页面。用户以后仍可批量选择深度策展、局部提取、仅保留原料或归档，系统不得因长期未处理而自动删除。
...
- `deep-curation`：完成原料归档后形成带范围、证据和成本边界的处理提案；三类 Draft Profile 只能用于方法实验，SOP-001 也只是未来完整地图路径的候选实现。
...
未来重构后的 SOP-001 或等价处理器只生成绑定范围与证据的派生/待审工件，不写可信知识；在其独立设计与实现门禁通过后，可以使用以下任一预授权触发，而无需逐篇先审核原文内容：

- 用户捕获时明确要求深度整理。
- 用户在后续审核中选择 `deep-curation`。
- 已存在一条用户可见、可撤销且范围精确的规则。

仅有模型对长度或价值的判断时，只能建议运行。没有触发授权的长文保持 `raw-source`，以后再批量处理。当前旧 SOP-001 仍受暂停边界约束，本节不构成运行授权。
...
- [ ] 用户明确深度整理或命中届时获批的人工规则时，系统只记录/提交深层处理意图；Profile/证据契约和 SOP-001 重构门禁通过后，才可生成派生或待审核工件，且不得直接执行 SOP-000B/SOP-002。
...
| 长文是否都自动跑 SOP-001 | 不以长度单独决定 | 用户明确深度整理或命中人工配置规则时进入深层处理路由；实际 Profile 由规则、风险、审核预算和用户需求共同决定；否则保留为 `raw-source`，模型只能建议 |
...
1. 实现拆解与编码执行方案已经批准；四个公开文本操作以及 C6A 业务事务崩溃恢复、C6B 派生状态重建和 C6C Store 迁移均已闭合。C7-0 `dbe8329` 与 C7A `fb61358` 已同步到 `origin/main`；最后一轮已登记的干净 Windows CI 仍为运行 `35319645501` 在 `ea8f84e` 上通过。C7B 四操作分派与安装入口已完成本地内容、300 项全量验证与本批独立版本化，但尚未 push。下一功能门禁为需单独授权的 C7V，随后是 C8；生产初始化与外部接入仍未授权。
