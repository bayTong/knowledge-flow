...
> 状态：Approved Design；C0–C3 已完成，`capture_text` 已通过本阶段验收但路由流程尚未完成<br>
...
1. 实现拆解与编码执行方案已经批准；C3 已完成，完整 `capture_text` 事务已在测试持有的 Store 中通过真实大小、加强版竞态与故障验收；R0.1/R0.2、D0-F、D0G、C4-0、R0.3D 与 R0.3F 也已闭合。C4A 读取契约能力、C4B `get_capture` 与 C4C `list_captures` 已分别以 `06cff02`、`666ba18`、`231ad09` 完成并 push；C4V 已完成 GET/LIST 全矩阵、2 项公共 API 组合验收和当前全量 214 项本地验证，并由独立本地提交闭合但尚未 push。C5-0 及后续批次未授权。
