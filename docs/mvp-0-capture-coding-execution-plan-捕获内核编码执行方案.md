...
> 状态：Approved Design；C7A 私有 CLI 协议能力已完成本地实现与验证，当前 293 项全量通过；下一功能门禁为 C7B 四操作适配与安装入口（未授权）<br>
> 整理日期：2026-09-02<br>
> 确认日期：2026-09-02<br>
...
> C7-0 CLI 契约收口日期：2026-09-21（独立本地提交 `dbe8329`；尚未 push）<br>
> C7A 协议能力与安全原语完成并独立版本化日期：2026-09-22（尚未 push）<br>
> 当前状态同步日期：2026-09-22<br>
...
> 执行进度：C0–C2 已闭合；C3A 已以 `346164d` 完成并在 105 项全量测试下验收，C3B 已以 `8050d88` 完成并在 122 项全量测试下验收，C3C 在 140 项全量测试下闭合事务，C3V 在 144 项全量测试下完成阶段验收；R0.1/R0.2 分别以 `92a37b3`、`79515ed` 完成，R0 后为 148 项；D0G 后 156 项，R0.3D 后 159 项，R0.3F 后 163 项；C4A 后为 182 项并以 `06cff02` 推送；C4B 后为 197 项并以 `666ba18` 推送；C4C 后为 212 项并以 `231ad09` 推送；C4V 后为 214 项并以 `1e38f2f` 推送；C5-0 `ea530ad` 与最小 Windows CI `c4d2c7b` 也已推送；C5A 后为 231 项，C5B 后为 250 项，C5V 后为 253 项，C6A 后为 258 项，C6B 后为 265 项，C6C 后为 274 项；截至 C6C `ea8f84e` 已同步到 `origin/main`，Windows CI 运行 `35319645501` 首次通过；C7-0 已以本地提交 `dbe8329` 封存，C7A 新增 19 项协议测试后当前全量为 293 项<br>
> 当前授权：用户已授权并完成 C7A 内容实施与独立本地提交；C7B、真实 Capture Store 和外部系统接入均未授权
...
完成记录（2026-09-17；独立提交 `f686941`，未 push）：新增管理模块 `recovery.py` 及显式 `rebuild_capture_store_derived_state`，不加入四个日常文本操作的顶层入口，也不加入 C7 CLI。操作先取得既有 Store 写锁，完整解析所有已提交版本/Event/Envelope并流式重算所有已提交 Payload 哈希，同时校验幂等 `scope + key_sha256` 全局唯一；任一不可变损坏都会在派生写入前失败。验证全部通过后，缺失、已知损坏或落后的 `capture.yaml` 才经同目录 UUIDv7 临时文件耐久写入、身份复核和 `os.replace` 单项原子替换；已有效投影不改写，进程在一次替换后 `os._exit(70)` 仍可由新进程只补剩余项。

C6B 没有发明磁盘幂等索引或 outbox job schema：`indexes/idempotency/` 恢复后仍为空，同 key 查询继续扫描不可变 Envelope；Envelope v1 的 `delivery_requests` 固定为空，因此 outbox 只恢复 `pending/running/failed/completed` 四个空 bucket。未知 projection 版本、未知索引/outbox 条目、非普通/reparse 目标均原样保留并失败关闭；未知 staging 与唯一未提交 N+1 尾部完全不触碰。成功结果只返回 Store ID 与扫描/验证/重建计数，不含写入操作专用的 `saved`、`commit_state` 或 `warnings`。

新增 `tests/capture/integration/test_derived_state_rebuild.py` 的 7 项测试后，捕获内核 250 项、维护与文档脚本 15 项，普通与严格全量共 265 项通过；覆盖 REC-01–REC-03、缺失/损坏/落后投影、重复运行、索引删除后的 capture/append 同 key 回执、原件损坏写前失败、未知格式保留、staging/尾部不修复和真实进程中断续建。固定工程检查与生产路径复核通过，本批以 `f686941` 独立闭合。

#### C6C：Store 迁移

- 显式管理操作 `migration.migrate_capture_store` 必须同时取得配置路径、源根、目标根和预期 Store ID；配置只能正指向源或已完成的目标，不允许“迁移当前随便连到的 Store”。操作不加入顶层四操作 API 或 C7 日常 CLI。
- 配置初始化锁覆盖整次迁移，源 Store 写锁冻结稳定事实；源在目标创建前完成 Manifest、所有已提交 Item/Version/Event/Envelope、全部已提交 Payload、投影及空派生目录验证。随后目标取得自己的 Store 写锁，防止另一个配置并发改写目标。
- 跨根始终使用不超过 1 MiB 的流式复制，不以跨卷 rename 冒充原子性。稳定快照逐文件记录大小和 SHA-256；目标只允许为空，或其普通非 reparse 目录/文件是源快照的精确兼容子集，任何外来、替换或损坏对象都原样保留并失败关闭。
- `.staging/` 内容属于未提交事务而不迁移；目标只建立空 `.staging/`。`journal/capture-write.lock` 是每个根自己的内核锁域，不复制锁状态。Item 下由 Event 真源隔离的未提交尾部仍属于稳定 Item 字节并原样复制；源根始终保留。
- 目标完成字节快照相等和同一套语义验证后，才将规范新配置写入配置同目录临时文件并用 `os.replace` 原子切换。替换前失败时配置仍指向源；替换已生效但进程退出时重放识别为已完成，只验证而不重复复制。
- `capture_text` 与 `append_capture_version` 在取得原 Store 写锁后、任何恢复或提交动作前重新读取同一配置；若迁移已切换到目标，旧请求返回 `capture_store_unavailable + not-committed`、`stage=config-binding-changed` 并只清理自己的源 staging，不能造成 A/B 分叉。只读请求已经选择 A 时可完成只读快照，不产生分叉。
- 完成 MIG-01–MIG-05；不创建或迁移生产 Store，只在测试持有的两个临时根之间演练。

完成记录（2026-09-18；本批完成时独立提交尚未 push）：新增 `migration.py` 与显式 `migrate_capture_store`，保持 `__init__.py` 的四操作公开面不变。9 项 `test_store_migration.py` 覆盖完整 A→B 身份/字节/四操作验收、首个目标文件后真实 `os._exit(70)` 与兼容续传、损坏部分目标和外来目标拒绝、配置 replace 失败回退、原子切换后真实进程退出与幂等重放、源原件写前失败，以及迁移切换前排队写请求的防分叉复核。捕获内核增至 259 项、全量增至 274 项；普通与严格 `ResourceWarning`、`compileall`、`pip check`、确定性文档护栏、diff 检查及生产路径复核均已通过。在本批完成时，下一功能门禁是尚未授权的 C7；本批自身不授权 push 或生产初始化。

远端门禁记录（2026-09-18）：用户随后授权并完成 C5A `63a3250`、C5B `ab2a613`、C5V `a9913e2`、C6A `84ee1d7`、C6B `f686941` 与 C6C `ea8f84e` 的 push；`origin/main` 已指向 `ea8f84e`。Windows CI 运行 `35319645501` 在干净 `windows-latest` / Python 3.13 runner 上首次通过普通与严格 `ResourceWarning` 全量测试、`compileall`、`pip check` 和确定性文档检查。该门禁不部署项目、不创建生产配置/Store，也不授权 C7。

### C7：机器适配 CLI

目标：给未来 Harness 或 UI 一个受限机器接口，不让调用方直接写 Store 文件。公开协议以[实现拆解与测试矩阵第 3.7 节](mvp-0-capture-implementation-plan-捕获内核实现拆解与测试矩阵.md#37-c7-v1-受限-cli-契约)为唯一 L2 细节权威，操作映射以[四操作契约第 7A 节](mvp-0-capture-operations-本地文本捕获操作契约.md#7a-c7-受限-cli-适配映射)为准。

#### C7-0：CLI 编码前契约收口

只修改文档及机器可读状态护栏中的当前门禁预期值，冻结以下会改变公共机器接口或安全边界的事项：

- console entry 与四个精确操作名；只允许可选绝对 `--config`，不提供 `--text`、管理操作或测试开关。
- 请求 schema/version、每操作字段白名单、65,536 byte 头上限、LF/CRLF、严格 JSON、UTF-8、正文长度和 EOF 规则。
- 写操作先做配置大小预检，再在核心调用前把完整正文以不超过 1 MiB 的块验证到 Store 外磁盘 spool，防止“已经提交后才发现尾随字节”。
- 统一响应 schema/version/`body_length_bytes`、get 头后正文顺序、0/2/70 三类精确退出码，以及 stderr/日志脱敏。
- 生产 `PathPolicy` 只能由可信安装上下文构造；临时 Store 的测试 policy 只能经不安装、不导出的私有 runner 注入。
- 将实现拆为 C7A、C7B、C7V，并冻结 CLI-01–CLI-26；不修改 Python 公共 API、Store schema、磁盘事实或四操作错误优先级。

完成记录（2026-09-21）：上述契约已同步到既有操作契约、实现矩阵、编码方案和冲突登记；五个状态锚点及 `test_doc_check.py` 的唯一当前门禁断言同步为 C7A，未增加、删除或弱化测试。未新增平行协议文档，未创建 `cli.py`、console entry、生产配置或生产 Store；随后以独立本地提交 `dbe8329` 封存，尚未 push。

#### C7A：协议能力与安全原语

主要文件：

- `src/knowledgeflow_capture/cli.py`
- `tests/capture/unit/test_cli_protocol.py`
- 必要的轻量 CLI golden fixture

只实现严格参数/JSON/字段解析、请求对象映射、配置大小预检、输入/输出临时 spool、规范响应编码和可注入二进制流的私有 runner。所有读取/写入块不得超过 1 MiB；所有临时对象必须由适配器单次调用独占并在可证明归属时清理。C7A 不修改 `pyproject.toml`、不安装命令、不对生产 Store 执行四操作；以 CLI-01–CLI-13、CLI-19、CLI-21–CLI-24 的确定性单元/缩小阈值测试为主。

C7A 必须单独复核和提交；完成后停下请求 C7B 授权。

完成记录（2026-09-22；内容、本地验证与独立版本化完成，尚未 push）：新增不导出、无 `main()` 的 `cli.py` 私有 runner，严格接受四个操作名和可选绝对 `--config`，拒绝重复 key、非有限数、孤立 surrogate、未知字段、头部 BOM/裸 CR/超限、正文短读/尾随 byte/BOM/非法 UTF-8；写帧先经受信 `PathPolicy` 和既有配置解析器预检声明上限，再以不超过 1 MiB 的块写入独占磁盘临时文件。`get_capture` 使用第二个临时文件隔离头与正文；响应保持核心 `to_dict()`、统一长度和 0/2/70/固定 stderr 语义。19 项确定性测试覆盖 CLI-01–CLI-13、CLI-17、CLI-19–CLI-24 的本批可证明部分，普通与严格 `ResourceWarning` 全量均为 293 项（捕获测试 278 项、脚本测试 15 项）；未修改 `pyproject.toml`、未安装命令、未执行真实四操作或创建生产配置/Store。下一功能门禁为需单独授权的 C7B。
...
- `src/knowledgeflow_capture/cli.py`
- `pyproject.toml` 的 `knowledgeflow-capture` console script
- `tests/capture/integration/test_cli.py`
- 测试包内、不进入发布接口的子进程 support

把已经验证的帧映射到 `capture_text`、`get_capture`、`list_captures`、`append_capture_version`，并从可信包/安装上下文构造生产 `PathPolicy`。功能测试只能对子进程注入 `PathPolicy.test_owned` 并写入测试持有的临时 Store；真实安装入口只做无写入 smoke test。适配器不得复制事务、扫描 Item、直接写 Store 或改变核心结果；以 CLI-14–CLI-19、CLI-21–CLI-25 为完成边界。

C7B 必须单独复核和提交；完成后停下请求 C7V 授权。

#### C7V：CLI 阶段独立验收

默认不再扩写生产能力，只补验收测试和事实记录：

- 完成 CLI-01–CLI-26，尤其真实 4 MiB/64 MiB 双向流、两个真实 CLI 进程同 key 竞争、尾随 byte 零提交、stdout 中途失败、退出码和敏感信息扫描。
- 在干净临时安装环境验证 console script 可发现，且测试 policy 不可由命令行、JSON 或环境变量触达。
- 重跑全部捕获/脚本测试的普通与严格 `ResourceWarning` 模式、`compileall`、`pip check` 和确定性文档检查。
- 检查默认机器配置、生产 Store 和仓库内容在验收前后不变。
...
| A2 后续批次 | 明确点名当前批次，例如“继续 C7A”；历史批次包括 C1/C2A/…/C7-0 | 只执行本次点名批次并在边界报告；分批名称不能一次授权自动跨越 | 未授权批次和范围扩张 |
