...
> 授权边界：2026-09-08 的本次确认只批准设计；后续 C3A–C3V、R0.1/R0.2、D0-F、D0G、C4-0、R0.3D、R0.3F、C4、C5-0、C5A–C5V 与 C6A–C6C 已分别完成；业务事务崩溃恢复、派生状态重建与 Store 迁移已通过定向验收，但 push、C7、生产配置和生产 Capture Store 仍需独立授权
...
3. C3A/C3B 已在后续独立停点完成，C3C/C3V 已于 2026-09-11 闭合并验收完整 `capture_text` 事务；C4 与 C5-0 也已闭合并版本化，最小 Windows CI 首次远端运行已通过。C5A/C5B/C5V 与 C6A–C6C 已完成；下一功能门禁为需单独授权的 C7 受限 CLI。
