...
> 授权边界：2026-09-08 的本次确认只批准设计；后续 C3A–C3V 已分别实施，C4、生产配置和生产 Capture Store 仍需独立授权
...
3. C3A/C3B 已在后续独立停点完成，C3C/C3V 已于 2026-09-11 在测试持有的临时 Store 中先后闭合并验收完整 `capture_text` 事务；只有用户另行明确授权 C4 后，才实现读取与列表操作。
