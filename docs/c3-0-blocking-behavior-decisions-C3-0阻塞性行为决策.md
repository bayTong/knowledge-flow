...
> 授权边界：2026-09-08 的本次确认只批准设计；后续 C3A–C3V、R0.1/R0.2、D0-F、D0G、C4-0、R0.3D、R0.3F 与 C4 已分别完成并版本化；C5-0 追加契约内容已完成、待版本化，C5A、生产配置和生产 Capture Store 仍需独立授权
...
3. C3A/C3B 已在后续独立停点完成，C3C/C3V 已于 2026-09-11 在测试持有的临时 Store 中先后闭合并验收完整 `capture_text` 事务，R0.1/R0.2 随后完成初始化所有权加固，D0-F、D0G、C4-0、R0.3D、R0.3F、C4A、C4B、C4C 与 C4V 也已闭合并版本化。C4V `1e38f2f`、C5-0 `ea530ad` 和最小 Windows CI `c4d2c7b` 均已 push，首次远端 CI 已通过；下一功能门禁为 C5A，仍需另行授权。
