...
3. C3A/C3B 已在后续独立停点完成，C3C/C3V 已于 2026-09-11 在测试持有的临时 Store 中先后闭合并验收完整 `capture_text` 事务，R0.1/R0.2 随后完成初始化所有权加固，D0-F、D0G、C4-0、R0.3D、R0.3F 与 C4A 也已闭合。下一门禁为需另行授权的 C4B。
...
C4A 记录：2026-09-13 在不修改顶层公开入口和 `operations.py` 的边界内完成读取 schema、模型、游标 codec、版本链纯原语与两版本 golden；新增 19 项后当前全量 182 项普通与严格模式均通过，并由独立本地提交闭环且未 push。
