...
> 授权边界：2026-09-08 的本次确认只批准设计；后续 C3A–C3V、R0.1/R0.2、D0-F、D0G 与 C4-0 已分别完成；R0.3D、C4 实现、生产配置和生产 Capture Store 仍需独立授权
...
3. C3A/C3B 已在后续独立停点完成，C3C/C3V 已于 2026-09-11 在测试持有的临时 Store 中先后闭合并验收完整 `capture_text` 事务，R0.1/R0.2 随后完成初始化所有权加固，D0-F、D0G 与 C4-0 也已闭合。下一步须另行授权并完成 R0.3D，之后才可另行授权 C4A。
