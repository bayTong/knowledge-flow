...
> 状态：Approved Design；四个公开文本操作、C5V 及 C6A–C6C 恢复/迁移能力已完成<br>
> 确认日期：2026-09-02<br>
...
> C5A 完成日期：2026-09-16（独立提交 `63a3250`；2026-09-18 已 push 至 `origin/main`）<br>
...
> C7-0 CLI 适配映射确认日期：2026-09-21；C7A 私有协议层完成并独立版本化日期：2026-09-22（`fb61358`，已 push）；C7B 四操作适配与安装入口完成日期：2026-09-24（本批独立版本化，尚未 push）<br>
> 适用范围：单机、单用户、纯文本捕获<br>
> 边界：本文定义调用方可见的四个日常文本操作及 C7 CLI 的一对一适配边界；C6A 不增加公开参数，C6B/C6C 的显式管理操作与四操作分离；C7A/C7B 已完成协议、四操作分派与安装入口，C7V、C8、生产目录和外部接入仍未完成
...
1. 管理调用必须显式提供配置路径、源根、目标根、预期 `store_id` 和路径策略；不从当前目录或目标 Manifest 猜测身份。
2. 在配置初始化锁和源 Store 锁内，先完整验证源 Manifest、所有已提交 Item/Version/Event/Envelope、全部 Payload 实际哈希、幂等唯一性、投影及派生目录。源验证失败时不创建目标。
3. 只复制稳定树：源 `.staging/` 内容和 `journal/capture-write.lock` 锁状态不迁移；已在 Item 内的未提交尾部按原字节复制，但仍由 Event 真源保持不可见。
4. 目标只能是空目录，或目录/文件是源的子集且已有文件字节数与 SHA-256 完全匹配的兼容部分目标。复制块不超过 1 MiB，每个新文件都经 flush、目录 flush 和回读哈希，不依赖跨卷 rename。
5. 在目标 Store 锁内完整复核目标字节快照和语义。只有完全一致时，才在配置同目录耐久写入临时文件并以 `os.replace` 切换根路径。
6. 迁移成功、失败或进程中断都不自动删除源。配置已指向目标时的同请求重放只重新验证源/目标，不再切换配置。

两个写操作在读取配置后可能等待旧 Store 锁，因此取得锁后、执行恢复或任何最终写入前必须重读配置。若 root 或两个大小阈值已变，请求返回 `capture_store_unavailable + not-committed`，内部阶段为 `config-binding-changed`，只清理本请求自有 staging，不回写旧根。读操作在切换前已选定旧根时可完成只读检查，不会导致 A/B 分叉。

只有已完整复制且哈希匹配的兼容部分目标才能续传。若进程在写入单个目标文件中途退出，遗留的部分文件会被视为目标冲突并失败关闭，C6C 不宣称自动覆盖或修复它。
...
`recovery.rebuild_capture_store_derived_state` 是显式维护入口，不是第五个日常文本操作，也不改变本章四个操作的签名、回执或动态 warning 语义。读取和幂等重试继续只报告当前投影事实，不静默修复。

维护入口在既有 Store 写锁内先完整验证所有已提交 Item/Version/Event/Envelope、全部已提交 Payload 实际哈希、幂等记录唯一性及派生目录格式；全部通过后，才逐项原子替换缺失、已知损坏或落后的 `capture.yaml`，并恢复 MVP-0 的空索引/outbox 目录。它不创建持久幂等索引或 outbox job，不修复损坏原件、未知 staging 或未提交尾部；未知机器版本/派生条目原样保留并失败关闭。成功回执是管理计数，不含 `saved`、`commit_state` 或 `warnings`。

### 7.11 C6C Store 迁移管理边界
