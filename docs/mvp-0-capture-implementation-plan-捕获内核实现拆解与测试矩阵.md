...
> 边界：本文定义实现与测试要求；C5B 内容与本地验证已完成并公开完整 append，但 C5V 尚未验收；当前不授权 C5V、生产 `E:\KnowledgeFlowData`、GBrain、LLM、KB 路由或 UI
...
- C5A 已实现并验证追加请求/结果、严格 Event writer、版本 2 state、独立 staging/清理、尾部身份与提交证据原语；C5B 已把这些原语集成为公开完整 `append_capture_version`，闭合全 Store 扫描、幂等/CAS、当前 Payload attestation、匹配尾部续封、版本/Event 无覆盖提交、最终回读和投影 warning。
- 捕获包已精确锁定 `PyYAML==6.0.3`；C0–C2 里程碑自动发现 85 项测试，稳定化后为 93 项，C3A 后为 105 项，C3B 后为 122 项，C3C 后为 140 项，C3V 后为 144 项，R0.1/R0.2 后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后为 231 项，C5B 后当前为 250 项。

### 1.2 尚不存在

- 没有 `package.json`、Node/Bun 应用或桌面前端。
- `append_capture_version` 的公开完整事务已经实现；真实双进程竞争、真实 4/64 MiB 新版本和完整 APP-01–APP-24 阶段验收留在 C5V，尚未授权。
...
| 自动化测试 | C5B 后当前全量 250 项通过（捕获内核 235 项、维护脚本 7 项、文档护栏 8 项） | 在 C5A 证据上新增 18 项端到端追加事务和 1 项跨目录同卷 Event 无覆盖提交回归，闭合 APP-04–APP-23 的单进程/确定性故障范围 |
...
| G0.5 编码方案 | **C0–C4V 与 C5-0 已逐批通过、版本化并 push；C5A/C5B 内容与本地验证已完成，下一功能门禁为 C5V** | C5V 及后续未授权批次的业务代码和真实 Store |
...
以上选择已确认，本文保持 `Approved Design`。C0–C2 里程碑为 85 项测试，稳定化后为 93 项；C3V 时为 144 项，R0.1/R0.2 后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后为 231 项，C5B 后当前为 250 项。C5B 已公开完整追加事务，但真实双进程、真实 4/64 MiB 和整阶段矩阵仍留给 C5V；下一功能门禁为 C5V，仍需另行明确授权。真实 `E:\KnowledgeFlowData\capture-store` 仍只有在 C8 通过后、用户另行明确要求“初始化生产 Capture Store”时才允许创建。
