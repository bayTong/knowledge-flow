...
> 边界：本文定义实现与测试要求；C7A/C7B 已实现受限 CLI 协议、生产四操作分派与安装入口，C7V、C8、生产 `E:\KnowledgeFlowData`、GBrain、LLM、KB 路由及 UI 尚未实施
...
以上方案及第 13 节九项技术选择已于 2026-09-02 获批。C0–C2（含 C2B-3 崩溃恢复）已逐批授权并完成；[C3-0 阻塞性行为决策](c3-0-blocking-behavior-decisions-C3-0阻塞性行为决策.md)已于 2026-09-08 获批，成功回执、固定错误消息与幂等命中警告语义于 2026-09-09 完成编码前收口。C3A–C6C 已逐批闭合并版本化；C5A `63a3250`、C5B `ab2a613`、C5V `a9913e2`、C6A `84ee1d7`、C6B `f686941` 与 C6C `ea8f84e` 已于 2026-09-18 同步到 `origin/main`，Windows CI 运行 `35319645501` 首次通过。C7-0 `dbe8329` 与 C7A `fb61358` 也已同步到 `origin/main`；C7B 于 2026-09-24 完成本地内容、验证与本批独立版本化，但尚未 push。这不授权 C7V、创建生产目录或接入外部系统。
...
- C6B 已增加与四个日常文本操作分离的显式管理操作：先完整验证所有已提交不可变事实与 Payload 哈希，再原子重建缺失/损坏/落后投影；空幂等/outbox 目录可恢复，未知格式、staging、尾部与损坏原件保持不动。
- C6C 已增加显式 `migration.migrate_capture_store`：绑定配置、源路径、目标路径和预期 Store ID；完整验证源后执行有界文件复制，只接受空目标或可证明兼容子集，目标完整复核后原子切换配置并保留源。两个写操作在取得 Store 锁后复核配置绑定，阻止迁移切换前排队的请求回写旧根。
- C7A 已新增私有 `cli.py` runner，在核心调用前完成严格命令/头/字段/正文/EOF 验证、受信配置上限预检和有界输入 spool，并在核心返回后以独立输出 spool 形成规范响应、固定退出码和脱敏 stderr；C7B 又增加零参数生产 `main()`、四操作一对一分派、可信生产 `PathPolicy`、固定 console entry 与仅测试代码可用的子进程 support。
- 捕获包已精确锁定 `PyYAML==6.0.3`；C0–C2 里程碑自动发现 85 项测试，稳定化后为 93 项，C3A 后为 105 项，C3B 后为 122 项，C3C 后为 140 项，C3V 后为 144 项，R0.1/R0.2 后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后为 231 项，C5B 后为 250 项，C5V 后为 253 项，C6A 后为 258 项，C6B 后为 265 项，C6C 后为 274 项，C7A 后为 293 项，C7B 后当前为 300 项。

### 1.2 尚不存在

- 没有 `package.json`、Node/Bun 应用或桌面前端。
- `append_capture_version` 的公开完整事务、APP-01–APP-24 阶段验收及 C6A–C6C 恢复、重建、迁移已经完成；C7A/C7B 受限 CLI 协议、生产分派和安装入口已经实现，C7V 与 C8 总验收尚未实现。
- 突然断电、控制器缓存与第三方长期占用仍不由 C6A–C6C 的进程终止测试证明。
- 已安装的 `knowledgeflow-capture` 目前只是受限机器协议入口；C7B 通过本地验证不代表 C7V 真实边界或 C8 总验收已经完成，也不代表生产 Store 已初始化。
...
| 自动化测试 | C7B 后当前全量 300 项通过（捕获测试 285 项、维护脚本 7 项、文档护栏 8 项） | C7A 新增 19 项覆盖严格帧与安全原语；C7B 新增 7 项覆盖真实四操作、安装入口、生产 policy 和测试能力隔离 |
| 生产 `capture-root` | 尚未创建 | 所有实现测试必须使用隔离临时目录 |

Python、Node.js 与 Bun 盘点来自 2026-09-02 至 2026-09-04；实现与测试状态已同步至 2026-09-22。它们都是本机事实，不是跨机器规范。
...
| `recovery.py` | C6B 显式派生状态重建管理操作 | 日常文本操作、索引/job 新 schema 或原件修复 |
...
| M0-V0 | C4V 读取阶段验收 | E8、E9 | **2026-09-14 已完成并由独立提交 `1e38f2f` 闭合、已 push：GET/LIST 全矩阵、真实 4/64 MiB 公共读取闭环、静态分页和页间受控新增通过；当时全量 214 项** |
...
| M0-D6 | C7-0 CLI 契约冻结 | E6–E11C | **2026-09-21 已冻结四命令、请求字段、严格帧、预验证 spool、统一响应头、退出码、脱敏及生产/测试 policy 隔离；本地提交 `dbe8329` 已封存设计且未创建入口** |
| M0-E12A | C7A CLI 协议能力与安全原语 | M0-D6 | **2026-09-22 已完成并以 `fb61358` 同步至 `origin/main`：严格参数/JSON/字段解析、请求映射、配置预检、输入/输出磁盘 spool、规范结果编码和私有可注入 runner 通过；19 项新增测试后全量 293 项** |
| M0-E12B | C7B 四操作适配与安装入口 | M0-E12A | **2026-09-24 已完成本地内容、验证与本批独立版本化：四操作 dispatch、生产 `PathPolicy`、`knowledgeflow-capture` entry 与缩小阈值子进程闭环通过；7 项新增测试后全量 300 项；不创建生产 Store；尚未 push** |
...
C6A 已以 5 个自动化测试方法覆盖 16 个真实子进程崩溃边界：capture 9 点、append 7 点；每点均由无故障钩子的新进程验证同 key 恢复与稳定重试。另行验证只清理可证明放弃且租约可取得的固定树，未知/旧式/额外对象/身份变化树原样保留，并在真实 Windows junction 与 symlink 下不跟随、不越界。C6B 又以 7 项测试闭合 REC-01–REC-03、重复运行、原件损坏写前失败、未知格式保留、staging/尾部不修复及一次投影替换后真实进程中断的新进程续建。C6C 以 9 项测试闭合 MIG-01–MIG-05、失败回退、幂等重放、复制/切换真实进程中断和旧写请求防分叉：

| ID | 场景 | 预期 |
|---|---|---|
| REC-01 | 删除 `capture.yaml` | **已通过：从版本和事件重建当前投影；损坏/落后投影同样恢复，有效投影不改写** |
| REC-02 | 删除幂等索引 | **已通过：恢复空目录并从不可变 Envelope 校验 key 映射；capture/append 同 key 重试仍返回原事实** |
| REC-03 | 删除空 outbox 投影 | **已通过：MVP-0 只恢复四个空 bucket，不创建 job 或第二真源** |
| MIG-01 | 将临时 Store 从路径 A 复制到 B | **已通过：`store_id`、Capture ID、稳定文件字节与哈希不变；跨根只复制，不依赖 rename** |
| MIG-02 | 复制未完成 | **已通过：真实进程在首个目标文件后退出，配置仍指向 A；兼容部分目标可由新进程续传** |
| MIG-03 | B 校验失败 | **已通过：损坏部分目标与外来非空目标均原样保留并拒绝切换，A 保持可读** |
...
| G0.5 编码方案 | **C0–C7A 已逐批通过并版本化，截至 `fb61358` 已 push；最后一轮已登记的干净 Windows CI 为 `35319645501` 在 `ea8f84e` 上通过；C7B 已完成本地内容与验证，下一功能门禁为未授权的 C7V** | C7V 及后续未授权批次的验收/业务代码和真实 Store |
...
以上选择已确认，本文保持 `Approved Design`。C0–C2 里程碑为 85 项测试，稳定化后为 93 项；C3V 时为 144 项，R0.1/R0.2 后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后为 231 项，C5B 后为 250 项，C5V 后为 253 项，C6A 后为 258 项，C6B 后为 265 项，C6C 后为 274 项，C7A 后为 293 项，C7B 后当前为 300 项。完整追加事务、真实双进程、真实 4/64 MiB、APP-01–APP-24、16 个业务进程崩溃边界、REC-01–REC-03 及 MIG-01–MIG-05 均已通过；C7A/C7B 的严格协议、安全 spool、真实四操作分派、生产 policy 与安装入口已通过确定性单元/缩小阈值子进程验证，下一功能门禁为需单独授权的 C7V。真实 `E:\KnowledgeFlowData\capture-store` 仍只有在 C8 通过后、用户另行明确要求“初始化生产 Capture Store”时才允许创建。
