...
> 边界：本文定义实现与测试要求；C6A/C6B 已闭合崩溃恢复与派生状态重建；C6C、生产 `E:\KnowledgeFlowData`、GBrain、LLM、KB 路由与 UI 尚未实施
...
- 捕获包已精确锁定 `PyYAML==6.0.3`；C0–C2 里程碑自动发现 85 项测试，稳定化后为 93 项，C3A 后为 105 项，C3B 后为 122 项，C3C 后为 140 项，C3V 后为 144 项，R0.1/R0.2 后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后为 231 项，C5B 后为 250 项，C5V 后为 253 项，C6A 后为 258 项，C6B 后当前为 265 项。

### 1.2 尚不存在

- 没有 `package.json`、Node/Bun 应用或桌面前端。
- `append_capture_version` 的公开完整事务、APP-01–APP-24 阶段验收、C6A 业务事务崩溃恢复及 C6B 投影/幂等/空 outbox 重建已经完成；C6C 迁移尚未实现。
- 没有迁移测试；突然断电、控制器缓存与第三方长期占用也不由 C6A/C6B 的进程终止测试证明。
...
| 自动化测试 | C6B 后当前全量 265 项通过（捕获内核 250 项、维护脚本 7 项、文档护栏 8 项） | C6B 新增 7 项，覆盖 REC-01–REC-03、写前完整验证、未知格式保留、staging/尾部不修复及真实进程中断续建 |
...
C6A 已以 5 个自动化测试方法覆盖 16 个真实子进程崩溃边界：capture 9 点、append 7 点；每点均由无故障钩子的新进程验证同 key 恢复与稳定重试。另行验证只清理可证明放弃且租约可取得的固定树，未知/旧式/额外对象/身份变化树原样保留，并在真实 Windows junction 与 symlink 下不跟随、不越界。C6B 又以 7 项测试闭合 REC-01–REC-03、重复运行、原件损坏写前失败、未知格式保留、staging/尾部不修复及一次投影替换后真实进程中断的新进程续建；下表中仅迁移场景仍属于 C6C：
...
| G0.5 编码方案 | **C0–C4V 与 C5-0 已逐批通过、版本化并 push；C5A/C5B/C5V/C6A/C6B 已在本地逐批完成，下一功能门禁为 C6C** | C7 及后续未授权批次的业务代码和真实 Store |
...
以上选择已确认，本文保持 `Approved Design`。C0–C2 里程碑为 85 项测试，稳定化后为 93 项；C3V 时为 144 项，R0.1/R0.2 后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后为 231 项，C5B 后为 250 项，C5V 后为 253 项，C6A 后为 258 项，C6B 后当前为 265 项。完整追加事务、真实双进程、真实 4/64 MiB、APP-01–APP-24、16 个业务进程崩溃边界及 REC-01–REC-03 均已通过；下一功能门禁为 C6C Store 迁移。真实 `E:\KnowledgeFlowData\capture-store` 仍只有在 C8 通过后、用户另行明确要求“初始化生产 Capture Store”时才允许创建。
