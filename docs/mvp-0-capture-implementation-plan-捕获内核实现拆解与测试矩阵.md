...
- 捕获包已精确锁定 `PyYAML==6.0.3`；C0–C2 里程碑自动发现 85 项测试，稳定化后为 93 项，C3A 后为 105 项，C3B 后为 122 项，C3C 后为 140 项，C3V 后为 144 项，R0.1/R0.2 后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后为 231 项，C5B 后为 250 项，C5V 后为 253 项，C6A 后为 258 项，C6B 后为 265 项，C6C 后当前为 274 项。

### 1.2 尚不存在

- 没有 `package.json`、Node/Bun 应用或桌面前端。
- `append_capture_version` 的公开完整事务、APP-01–APP-24 阶段验收及 C6A–C6C 恢复、重建、迁移已经完成；受限 CLI 与 C8 总验收尚未实现。
...
| 自动化测试 | C6C 后当前全量 274 项通过（捕获内核 259 项、维护脚本 7 项、文档护栏 8 项） | C6C 新增 9 项，覆盖 MIG-01–MIG-05、复制/切换真实进程中断、失败回退、幂等重放与旧写请求竞态 |
| 生产 `capture-root` | 尚未创建 | 所有实现测试必须使用隔离临时目录 |

Python、Node.js 与 Bun 盘点来自 2026-09-02 至 2026-09-04；实现与测试状态已同步至 2026-09-18。它们都是本机事实，不是跨机器规范。
...
| G0.5 编码方案 | **C0–C4V 与 C5-0 已逐批通过、版本化并 push；C5A–C6C 已在本地逐批完成，下一功能门禁为未授权的 C7** | C7 及后续未授权批次的业务代码和真实 Store |
