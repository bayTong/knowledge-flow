...
> 状态：Approved Design；C0–C2B-2 已完成，C2B-3 尚未授权<br>
> 确认日期：2026-09-02<br>
...
> 边界：本文定义实现与测试要求；不授权 C2B-3、生产 `E:\KnowledgeFlowData`、GBrain、LLM、KB 路由或 UI
...
以上方案及第 13 节九项技术选择已于 2026-09-02 获批。C0–C2B-2 已逐批授权并完成；这不自动授权 C2B-3、创建生产目录、提交 Git 或接入外部系统。
...
- 捕获包已精确锁定 `PyYAML==6.0.3`；自动发现共通过 76 项测试，其中 75 项覆盖 C1–C2B-2，另 1 项为包导入 smoke test。

### 1.2 尚不存在

- 没有 `package.json`、Node/Bun 应用或桌面前端。
- 没有任何四操作实现。
- 没有初始化故障注入、四操作、恢复或迁移测试；当前正常初始化通过不代表崩溃恢复已经验收。
- 没有统一 CLI；C1–C2B-2 与初始化测试通过不代表捕获业务行为已经实现。
...
| 自动化测试 | C0–C2B-2 自动发现并通过 76 项测试 | 已覆盖确定性基础原语、配置、路径、Manifest、初始化锁、durability 和正常 Store 初始化；尚无故障注入与四操作回归保障 |
| 生产 `capture-root` | 尚未创建 | 所有实现测试必须使用隔离临时目录 |

这些是 2026-09-02 至 2026-09-03 的本机事实，不是跨机器规范。
...
实际结果（2026-09-03）：INIT-01–16 已由 16 项初始化集成测试逐项覆盖并全部通过，另增加 3 项配置路径边界及 Windows 可信解析结果回归测试；加上 C0–C2B-1 的 57 项，当前自动发现总计 76 项。同请求并发与异 root 竞争分别额外连续复跑 100 次和 50 次，均保持单一身份、无覆盖和无孤儿目标。实现只在测试持有根内创建 Store、配置、锁和事务对象，未加入故障注入点，未创建真实配置或生产 Store，也未产生 Capture、State Event、outbox job、网络请求或 GBrain 调用。全量测试在 `ResourceWarning` 严格模式通过，`compileall`、依赖完整性和 `git diff --check` 同时通过。C2B-2 已完成，当前停在 C2B-3 授权门禁。
...
| G0.5 编码方案 | **C0–C1 已于 2026-09-02 通过；C2A、C2B-1、C2B-2 已于 2026-09-03 通过；当前停在 C2B-3 门禁** | 未授权批次的业务代码和真实 Store |
...
| G2B 初始化 | **C2B-1 的 LOCK/DUR 与 C2B-2 的 INIT 已全绿；仍待 C2B-3 的 FI** | 使用真实生产 root |
...
以上选择已确认，本文保持 `Approved Design`。C0–C2B-2 已完成并通过 76 项自动化测试；正常 Store 初始化已经实现，但崩溃恢复与四个捕获操作仍未完成，C2B-3 编码尚未授权。真实 `E:\KnowledgeFlowData\capture-store` 仍只有在用户另行明确要求“初始化生产 Capture Store”后才允许创建。
