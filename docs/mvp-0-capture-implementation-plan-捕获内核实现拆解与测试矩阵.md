...
> 边界：本文定义实现与测试要求；C7A 只实现不安装、不导出的 CLI 协议层，C7B/C7V 与 C8、生产 `E:\KnowledgeFlowData`、GBrain、LLM、KB 路由及 UI 尚未实施
...
以上方案及第 13 节九项技术选择已于 2026-09-02 获批。C0–C2（含 C2B-3 崩溃恢复）已逐批授权并完成；[C3-0 阻塞性行为决策](c3-0-blocking-behavior-decisions-C3-0阻塞性行为决策.md)已于 2026-09-08 获批，成功回执、固定错误消息与幂等命中警告语义于 2026-09-09 完成编码前收口。C3A–C6C 已逐批闭合并版本化；C5A `63a3250`、C5B `ab2a613`、C5V `a9913e2`、C6A `84ee1d7`、C6B `f686941` 与 C6C `ea8f84e` 已于 2026-09-18 同步到 `origin/main`，Windows CI 运行 `35319645501` 首次通过。C7-0 已于 2026-09-21 以本地提交 `dbe8329` 封存；C7A 于 2026-09-22 完成本地内容、验证与独立版本化，但尚未 push。这不授权 C7B、创建生产目录或接入外部系统。
...
- C7A 已新增私有 `cli.py` runner：在核心调用前完成严格命令/头/字段/正文/EOF 验证、受信配置上限预检和有界输入 spool；在核心返回后以独立输出 spool 形成规范响应、固定退出码和脱敏 stderr。该模块无 `main()`、不导出、不安装、不自行调用真实四操作。
- 捕获包已精确锁定 `PyYAML==6.0.3`；C0–C2 里程碑自动发现 85 项测试，稳定化后为 93 项，C3A 后为 105 项，C3B 后为 122 项，C3C 后为 140 项，C3V 后为 144 项，R0.1/R0.2 后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后为 231 项，C5B 后为 250 项，C5V 后为 253 项，C6A 后为 258 项，C6B 后为 265 项，C6C 后为 274 项，C7A 后当前为 293 项。

### 1.2 尚不存在

- 没有 `package.json`、Node/Bun 应用或桌面前端。
- `append_capture_version` 的公开完整事务、APP-01–APP-24 阶段验收及 C6A–C6C 恢复、重建、迁移已经完成；C7A 私有协议层已经实现，C7B 生产分派/安装、C7V 与 C8 总验收尚未实现。
- 突然断电、控制器缓存与第三方长期占用仍不由 C6A–C6C 的进程终止测试证明。
- 没有已安装的统一 CLI；C7A 私有 runner 通过测试不代表生产 entry 或四操作分派已经实现。
...
| 自动化测试 | C7A 后当前全量 293 项通过（捕获测试 278 项、维护脚本 7 项、文档护栏 8 项） | C7A 新增 19 项，覆盖严格帧、映射、预检、spool、规范输出、退出码、脱敏、故障与私有测试边界 |
...
| M0-E12A | C7A CLI 协议能力与安全原语 | M0-D6 | **2026-09-22 已完成本地内容、验证与独立版本化：严格参数/JSON/字段解析、请求映射、配置预检、输入/输出磁盘 spool、规范结果编码和私有可注入 runner 通过；19 项新增测试后全量 293 项，不安装 console entry；尚未 push** |
...
| G0.5 编码方案 | **C0–C6C 已逐批通过并版本化，截至 `ea8f84e` 已 push 且 Windows CI `35319645501` 首次通过；C7-0 已以 `dbe8329` 本地封存，C7A 已完成本地内容与验证，下一功能门禁为未授权的 C7B** | C7B 及后续未授权批次的业务代码和真实 Store |
...
以上选择已确认，本文保持 `Approved Design`。C0–C2 里程碑为 85 项测试，稳定化后为 93 项；C3V 时为 144 项，R0.1/R0.2 后为 148 项，D0G 后为 156 项，R0.3D 后为 159 项，R0.3F 后为 163 项，C4A 后为 182 项，C4B 后为 197 项，C4C 后为 212 项，C4V 后为 214 项，C5A 后为 231 项，C5B 后为 250 项，C5V 后为 253 项，C6A 后为 258 项，C6B 后为 265 项，C6C 后为 274 项，C7A 后当前为 293 项。完整追加事务、真实双进程、真实 4/64 MiB、APP-01–APP-24、16 个业务进程崩溃边界、REC-01–REC-03 及 MIG-01–MIG-05 均已通过；C7A 的不安装私有协议层已通过确定性单元/缩小阈值验证，下一功能门禁为需单独授权的 C7B。真实 `E:\KnowledgeFlowData\capture-store` 仍只有在 C8 通过后、用户另行明确要求“初始化生产 Capture Store”时才允许创建。
