...
> 状态：Approved Design；C0–C2 与 C3A–C3C 已完成，停在 C3V<br>
> 确认日期：2026-09-02<br>
...
> 边界：本文定义实现与测试要求；C3C 的完成不授权 C3V、C4–C5 其余三个捕获操作、生产 `E:\KnowledgeFlowData`、GBrain、LLM、KB 路由或 UI
...
以上方案及第 13 节九项技术选择已于 2026-09-02 获批。C0–C2（含 C2B-3 崩溃恢复）已逐批授权并完成；[C3-0 阻塞性行为决策](c3-0-blocking-behavior-decisions-C3-0阻塞性行为决策.md)已于 2026-09-08 获批，成功回执、固定错误消息与幂等命中警告语义于 2026-09-09 完成编码前收口。C3A 与 C3B 已于 2026-09-10 分别完成，C3C 已于 2026-09-11 完成；这仍不自动授权 C3V、创建生产目录或接入外部系统。
...
- C3A 已实现 Event/Projection v1、输入/时间/渠道边界、幂等摘要和精确回执，提交为 `346164d`；C3B 已实现 Store 写锁、有界 UTF-8 写入和可验证归属的 staging，提交为 `8050d88`；C3C 已实现公开 `capture_text` 的完整 T0–T9 事务。
- 捕获包已精确锁定 `PyYAML==6.0.3`；C0–C2 里程碑自动发现 85 项测试，稳定化后为 93 项，C3A 后为 105 项，C3B 后为 122 项，C3C 后全量为 140 项。

### 1.2 尚不存在

- 没有 `package.json`、Node/Bun 应用或桌面前端。
- 其余三个公开操作 `get_capture`、`list_captures`、`append_capture_version` 尚未实现。
- 没有完整 C3V、追加故障注入、投影与幂等索引恢复或迁移测试；初始化崩溃恢复已验收，C3C 事务故障分支已覆盖，但捕获业务崩溃恢复尚未验收。
- 没有统一 CLI；C3C 的 Python 操作边界通过测试不代表机器适配层已经实现。
...
| 自动化测试 | C3C 后全量 140 项通过（捕获内核 133 项、维护脚本 7 项） | 已覆盖 C3A/C3B 支撑能力和 C3C 缩小阈值 T0–T9 事务分支；真实 4/64 MiB 与加强版竞态/故障验收留在 C3V |
...
| G0.5 编码方案 | **C0–C1 已于 2026-09-02 通过；C2A、C2B-1、C2B-2 已于 2026-09-03 通过；C2B-3 已于 2026-09-04 通过；C3A/C3B 已于 2026-09-10 通过；C3C 已于 2026-09-11 通过；当前停在 C3V 门禁** | 未授权批次的业务代码和真实 Store |
...
以上选择已确认，本文保持 `Approved Design`。C0–C2 里程碑为 85 项测试，稳定化后为 93 项；C3A 契约能力与 C3B 写入基础分别以 `346164d`、`8050d88` 完成，C3C 完成公开 `capture_text` 事务后，全量为 140 项。安全 Store 初始化、初始化崩溃恢复、C3 支撑原语及缩小阈值下的完整 T0–T9 事务已经实现；C3V、其余三个公开操作和业务事务恢复仍未完成，当前停在 C3V 授权前。真实 `E:\KnowledgeFlowData\capture-store` 仍只有在用户另行明确要求“初始化生产 Capture Store”后才允许创建。
