...
> 状态：Approved Design；C0–C3、R0.1/R0.2、D0-F、D0G、C4-0 与 R0.3D 已完成；R0.3D 已通过独立本地提交闭合且未 push，当前 159 项测试通过；随后停在 R0.3F 授权门禁<br>
> 确认日期：2026-09-02<br>
...
以上方案及第 13 节九项技术选择已于 2026-09-02 获批。C0–C2（含 C2B-3 崩溃恢复）已逐批授权并完成；[C3-0 阻塞性行为决策](c3-0-blocking-behavior-decisions-C3-0阻塞性行为决策.md)已于 2026-09-08 获批，成功回执、固定错误消息与幂等命中警告语义于 2026-09-09 完成编码前收口。C3A 与 C3B 已于 2026-09-10 分别完成，C3C、C3V、R0.1 与 R0.2 已于 2026-09-11 先后完成，D0-F、D0G、C4-0 与 R0.3D 已分别于 2026-09-12、2026-09-13 完成版本化收口。这些动作都不自动授权 R0.3F、C4 实现、创建生产目录或接入外部系统。
...
- 捕获包已精确锁定 `PyYAML==6.0.3`；C0–C2 里程碑自动发现 85 项测试，稳定化后为 93 项，C3A 后为 105 项，C3B 后为 122 项，C3C 后为 140 项，C3V 后为 144 项，R0.1/R0.2 后为 148 项，D0G 当前全量为 156 项。
...
| 自动化测试 | D0G 当前全量 156 项通过（捕获内核 141 项、维护脚本 7 项、文档护栏 8 项） | 已覆盖 C3 T0–T9、真实 4/64 MiB、有界非 seekable 输入、锁边界同 key 竞态、加强版故障证据、R0 初始化所有权回归及确定性文档漂移 |
...
| M0-D4 | R0.3D 初始化错误语义诊断 | E6、M0-D3 | **2026-09-13 已通过独立本地提交闭合且未 push：3 项特征测试复现 C-032–C-034，并冻结未知候选、错误优先级和 WinError 分类** |
| M0-E6F | R0.3F 初始化错误语义修复 | M0-D4 | 按 C-032–C-034 完成局部修复，把 3 项特征测试转换为目标行为回归；未授权 |
| M0-E8A | C4A 读取侧契约能力 | M0-E6F | 追加 Event schema、连续版本发现、请求/结果模型、游标 codec 与真实两版本 golden 闭环；不公开读取操作 |
...
R0.3D 特征证据与 R0.3F 目标：

| ID | R0.3D 当前事实 | R0.3F 目标 |
|---|---|---|
| R03D-01（M1） | 未知合法命名事务候选仅一次 `stat` 失败，会让已有合法 Store 的幂等重开返回不可重试 `capture_store_unavailable`、`stage=path-stat`；既有 INIT-17 证明 `unlink` 原生错误仍能到达清理 handler | 未证明 marker/request 归属前的不可读候选保守跳过且不删除；已证明自有后的 stat/unlink/rmdir 失败继续保留 marker 并失败关闭 |
| R03D-02（M3） | 原始 `file-readback` 失败后的清理身份失败，会把公共 `stage` 覆盖为 `transaction-cleanup-identity` | 原错误的 `code/cause_code/retryable/stage` 不变，次级清理阶段仅进入安全 `details.cleanup_stage`；清理是唯一错误时仍作为主 stage |
| R03D-03（M4） | WinError 32/33/5 与仅 `errno.EACCES` 在 durability 映射中当前都不可重试 | 默认不可重试，仅 32/33 为可重试；5、仅 EACCES、无原因、校验失败及未知错误保持不可重试，锁等待分类不外推 |
...
| G0.5 编码方案 | **C0–C1 已于 2026-09-02 通过；C2A、C2B-1、C2B-2 已于 2026-09-03 通过；C2B-3 已于 2026-09-04 通过；C3A/C3B 已于 2026-09-10 通过；C3C/C3V 与 R0.1/R0.2 已于 2026-09-11 通过；D0-F 已于 2026-09-12 通过；D0G、C4-0 与 R0.3D 已于 2026-09-13 通过；随后停在 R0.3F 授权前** | 未授权批次的业务代码和真实 Store |
...
以上选择已确认，本文保持 `Approved Design`。C0–C2 里程碑为 85 项测试，稳定化后为 93 项；C3A 契约能力与 C3B 写入基础分别以 `346164d`、`8050d88` 完成，C3C 完成公开 `capture_text` 事务，C3V 完成独立阶段验收时全量为 144 项；R0.1/R0.2 初始化所有权加固后为 148 项，D0-F、D0G、C4-0 与 R0.3D 也已闭合；R0.3D 新增 3 项特征测试后当前为 159 项。安全 Store 初始化、初始化崩溃恢复及 C3 `capture_text` 已实现并通过本阶段验收；三个读取/追加公开操作和业务事务恢复仍未实现。下一门禁是另行授权 R0.3F，完成后才可授权 C4A。真实 `E:\KnowledgeFlowData\capture-store` 仍只有在用户另行明确要求“初始化生产 Capture Store”后才允许创建。
