...
C7-0 与 C7A 已随 `fb61358` 同步到 `origin/main`；C7B 的四操作分派、生产 `PathPolicy` 与 `knowledgeflow-capture` 安装入口已完成本地实现，普通/严格全量均为 300 项并随本批独立版本化，尚未 push。下一功能门禁是需要单独授权的 C7V。本页不授权 C7V、C8、生产初始化或外部接入。
