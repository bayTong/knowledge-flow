...
2. [MVP-0 捕获内核编码执行方案](mvp-0-capture-coding-execution-plan-捕获内核编码执行方案.md)已经批准，C0 测试骨架与 C1 确定性基础原语已经完成并通过 30 项自动化测试；C2 的阻塞性契约也已复核闭合。下一步仍需明确授权 C2A，先实现配置、路径和 Manifest；复核通过后再明确授权 C2B，实现锁、durability、初始化、重开、并发和故障恢复。
