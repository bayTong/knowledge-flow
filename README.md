...
> **Current status (2026-09-16):** the project is migrating from the legacy direct-initialization/curated-write workflow to a governed flow: local durable capture → human routing → proposal → exact approval → reversible write. C4V read-stage acceptance commit `1e38f2f` and C5-0 contract commit `ea530ad` are synchronized with `origin/main`. The complete GET-01–GET-16 and LIST-01–LIST-19 matrices remain green, while two additional public-API acceptance tests prove exact 4 MiB/64 MiB `capture_text → list_captures → get_capture` round trips, stable receipts, bounded I/O, static keyset traversal, and controlled creation between pages. C5-0 froze the append request/result, idempotency-before-CAS order, narrow same-request tail completion, Event-rename commit evidence, failure priority, and APP-01–APP-24 without changing production code or disk schema. Minimal Windows CI commit `c4d2c7b` has also been pushed; its first remote run `35075692046` passed the ordinary and strict suites plus compile, dependency, and documentation checks on a clean Windows runner. The suite remains at 214 tests (199 capture-kernel tests, 7 maintenance-script regressions, and 8 document-guard regressions). The next functional gate is separately authorized C5A; append and the production Store remain unimplemented. See the [`design authority and conflict register`](docs/design-authority-and-conflict-register-设计权威与冲突登记.md). Legacy SOP-002 and its write prompt are suspended.
