...
**前提一**：高质量知识策展需要领域判断力。你能区分核心概念和次要细节，能识别两个看似不同的说法实质上指向同一个概念——这些判断只有真正理解领域的人才能做。

**前提二**：用户之所以需要知识管理工具，恰恰因为不熟悉领域。如果已经懂了，直接写笔记就行，不需要 LLM 帮你提取和组织。

悖论在此：**策展的责任人在人类这边（因为只有人类知道自己的目标和语境），但策展所需的知识提取能力在 LLM 那边（因为只有 LLM 能穷举扫描长文本不遗漏）**。两条各自成立的前提指向了一个矛盾——谁来做策展？
...
- **LLM 负责穷举提取**——不筛选、不判断重要性（C5 硬约束）。每条提取锚定到原文位置（C2），不确定的地方显式标记（C3），建议严格隔离在事实之外（C4）。产出是一份结构化阅读地图
- **人类负责语义判断**——在阅读地图上逐条标记「确认入库」「忽略」「待更多原料」。你不需要信任 LLM 的判断力，你只需要信任它确实提取了所有东西（可通过逐段覆盖率和原文引用验证）
